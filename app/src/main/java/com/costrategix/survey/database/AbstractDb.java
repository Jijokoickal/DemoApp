...
public abstract class AbstractDb {
    ...
    private long insert(final String tableName, final ContentValues cv) {
        long id = this.appDatabase.insert(tableName, null, cv);
        return id;
    }
    ...
    //method to get the item count in the database table
    public int getCount() {
        int count = 0;
        final String selectQuery = " select count(" + getPrimaryKey() + ") from " + getTableName();
